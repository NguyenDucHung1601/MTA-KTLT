...
// CAU TRUC DU LIEU STACK SD DSLK KEP
...
int main(){
	...
	printf("-- Exercise 10: STACK (use Double linked list) _ NguyenDucHung --\n");
	int choice,value,maxsize;
	
	printf("\nMax size of Stack: "); 	scanf("%d",&maxsize);
	int a[maxsize];
	creatData(a,maxsize);
	writeData("input10.txt",a,maxsize);
	readData("input10.txt",a,maxsize);
	
	// ghi ket qua ra file output
	FILE *out;
	out = fopen("output10.txt", "a");
	//
