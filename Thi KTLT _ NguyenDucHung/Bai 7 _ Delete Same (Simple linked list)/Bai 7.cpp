...
int main(){
	...
	printf("-- Exercise 7: DELETE SAME (in Ascending simple linked list) _ NguyenDucHung --\n\n");
	printf(" - Number of List element : ");
	scanf("%d",&n);
	int a[n];
	creatData(a,n);
	writeData("input7.txt",a,n);
	readData("input7.txt",a,n);
	deleteSameValue();
...
void creatData(int a[], int n){
	for(int i=0; i<n; i++){
		a[i] = rand()%100+1;  // gtri ngau nhien trong doan 1->100
	}
}
